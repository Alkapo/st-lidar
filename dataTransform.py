...
def distribution_Plots(distribution, title='Resolution', results_path='../dataTransformed'):# Distribution parameters
...
if __name__ == '__main__':
    ...
    distribution_Plots(train_df['Resolution'], title='Resolution', results_path=results_path)

    # ------------ Price normalisation -----------------------------
    # Log transformation of sale price
    train_df["SalePriceLog"] = np.log1p(train_df["Resolution"])
    print(train_df.head())
    ...
    all_data.drop(['Resolution', 'SalePriceLog'], axis=1, inplace=True)
    print(all_data.head())
    ...
    # Correlation map to see how features are correlated with Resolution
    ...
    train_trans['Resolution'] = np.array(train_df['Resolution'])
